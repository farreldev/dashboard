...
const build = series(
  ...
  parallel(
    lint,
    series(parallel(puggs, styles, scripts, modernizr), html),
    images,
    fonts,
    extras
  ),
...
let serve;
if (isDev) {
  serve = series(clean, parallel(puggs, styles, scripts, modernizr, fonts), startAppServer);
} else if (isTest) {
  serve = series(puggs, clean, scripts, startTestServer);
} else if (isProd) {
